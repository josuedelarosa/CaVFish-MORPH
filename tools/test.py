...
def main():
    ...
    # -------------------- Guardar predicciones si se usa --dump --------------------
    if args.dump:
        results = []
        model = runner.model
        model.eval()  # <- reemplazo de switch_to('test')
        ...
        mmengine.dump(results, args.dump)
        print(f"✅ Predictions saved to: {args.dump}")
        return

    # -------------------- Testeo normal si no hay --dump --------------------
    runner.test()
